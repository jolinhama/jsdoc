...
var baseTags = exports.baseTags = {
    ...
    directive: {
        onTagged: function(doclet, tag) {
            doclet.addTag('kind', 'directive');

            // handle special case where both @class and @constructor tags exist in same doclet
            if (tag.originalTitle === 'directive') {
                // multiple words after @class?
                var looksLikeDesc = (tag.value || '').match(/\S+\s+\S+/);
                if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) &&
                    !/@classdesc\b/i.test(doclet.comment)) {
    ...
    service: {
        onTagged: function(doclet, tag) {
            doclet.addTag('kind', 'service');

            // handle special case where both @class and @constructor tags exist in same doclet
            if (tag.originalTitle === 'service') {
                // multiple words after @class?
                var looksLikeDesc = (tag.value || '').match(/\S+\s+\S+/);
                if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) &&
                    !/@classdesc\b/i.test(doclet.comment)) {
    ...
    provider: {
        onTagged: function(doclet, tag) {
            doclet.addTag('kind', 'provider');

            // handle special case where both @class and @constructor tags exist in same doclet
            if (tag.originalTitle === 'provider') {
                // multiple words after @class?
                var looksLikeDesc = (tag.value || '').match(/\S+\s+\S+/);
                if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) &&
                    !/@classdesc\b/i.test(doclet.comment)) {
    ...
    factory: {
        onTagged: function(doclet, tag) {
            doclet.addTag('kind', 'factory');

            // handle special case where both @class and @constructor tags exist in same doclet
            if (tag.originalTitle === 'factory') {
                // multiple words after @class?
                var looksLikeDesc = (tag.value || '').match(/\S+\s+\S+/);
                if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) &&
                    !/@classdesc\b/i.test(doclet.comment)) {
    ...
    filter: {
        onTagged: function(doclet, tag) {
            doclet.addTag('kind', 'filter');

            // handle special case where both @class and @constructor tags exist in same doclet
            if (tag.originalTitle === 'filter') {
                // multiple words after @class?
                var looksLikeDesc = (tag.value || '').match(/\S+\s+\S+/);
                if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) &&
                    !/@classdesc\b/i.test(doclet.comment)) {
    ...
    component: {
        onTagged: function(doclet, tag) {
            doclet.addTag('kind', 'component');

            // handle special case where both @class and @constructor tags exist in same doclet
            if (tag.originalTitle === 'component') {
                // multiple words after @class?
                var looksLikeDesc = (tag.value || '').match(/\S+\s+\S+/);
                if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) &&
                    !/@classdesc\b/i.test(doclet.comment)) {
    ...
    controller: {
        onTagged: function(doclet, tag) {
            doclet.addTag('kind', 'controller');

            // handle special case where both @class and @constructor tags exist in same doclet
            if (tag.originalTitle === 'controller') {
                // multiple words after @class?
                var looksLikeDesc = (tag.value || '').match(/\S+\s+\S+/);
                if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) &&
                    !/@classdesc\b/i.test(doclet.comment)) {
